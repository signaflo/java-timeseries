...
public class ArimaCoefficientsSpec {
    ...
    public void whenDifferencesWithMeanSetThenIllegalState() {
        ...
        ArimaCoefficients.newBuilder().setDifferences(1).setMean(50.0).build();
    }

    @Test
    public void whenMoreThanOneDifferenceWithDriftSetThenIllegalState() {
        exception.expect(IllegalStateException.class);
        ArimaCoefficients.newBuilder().setDifferences(1).setSeasonalDifferences(1).setDrift(50.0).build();
    }

    @Test
    public void testArimaCoefficientsEqualsAndHashCode() {
        ArimaCoefficients coeffs1 = ArimaCoefficients.newBuilder().setARCoeffs(0.3).build();
        ArimaCoefficients coeffs2 = ArimaCoefficients.newBuilder().setMACoeffs(-0.2).build();
        ArimaCoefficients coeffs3 = ArimaCoefficients.newBuilder().setARCoeffs(0.3).build();

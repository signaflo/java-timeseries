...
public final class MeanForecast {
  ...
  public MeanForecast(final MeanModel model, final int steps, final double alpha) {
    this.model = model;
  ...
  public final void plot() {
    new Thread(() -> {
      ...
      double fcstStdError = model.timeSeries().stdDeviation() * Math.sqrt(1 + (1/model.timeSeries().n()));
      double critValue = new StudentsT(model.timeSeries().n() - 1).quantile(1 - alpha/2);
      double[] errors = new double[forecast.n()];
      for (int t = 0; t < errors.length; t++) {
        errors[t] = critValue * fcstStdError;
      }
